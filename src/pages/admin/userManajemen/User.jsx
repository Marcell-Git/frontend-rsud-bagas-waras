...
const User = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nama_lengkap || !formData.username || (!editingItem && !formData.password) || !formData.role) {
  ...
  const openModal = (item = null) => {
    if (item) {
      ...
      setFormData({
        ...
        role: item.role || "",
        password: "", // Always clear password on edit open
      });
    } else {
      setEditingItem(null);
      setFormData({
    ...
    <div className="space-y-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 font-sans">
        <div className="font-sans">
          ...
          <p className="text-slate-500 mt-2 font-medium font-sans">
            Kelola akses akun administrator dan petugas sistem.
          </p>
        </div>
        <button
          onClick={() => openModal()}
          className="flex items-center justify-center gap-2 bg-slate-900 text-white px-8 py-3.5 rounded-2xl font-bold hover:shadow-xl hover:shadow-slate-900/20 transition-all active:scale-95 font-sans"
        >
          <Plus size={20} />
          Tambah User
        </button>
        ...
        <div className="overflow-x-auto font-sans">
          <table className="w-full text-left border-collapse font-sans">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100 text-slate-500 text-xs font-sans">
                <th className="py-6 px-8 font-black uppercase tracking-widest font-sans">User Info</th>
                <th className="py-6 px-8 font-black uppercase tracking-widest font-sans">Username</th>
                <th className="py-6 px-8 font-black uppercase tracking-widest font-sans">Role</th>
                <th className="py-6 px-8 font-black uppercase tracking-widest text-center font-sans">Aksi</th>
              </tr>
            </thead>
                ...
                >
                  <td className="py-5 px-8 text-slate-600 font-sans">
                    <div className="flex items-center gap-4 font-sans">
                      ...
                      <div className="font-sans">
                        ...
                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-tight font-sans">
                          ID: #{item.id}
                        </p>
                      </div>
                    </div>
                  ...
                  <td className="py-5 px-8 font-sans">
                    <div className="flex items-center gap-2 text-slate-700 font-bold font-sans">
                      <Fingerprint size={16} className="text-slate-400 font-sans" />
                      {item.username}
                    </div>
                  </td>
                  <td className="py-5 px-8 text-slate-600 font-sans">
          ...
          {users.length === 0 && (
            <div className="py-24 text-center font-sans">
              ...
              <p className="text-slate-500 font-bold font-sans">Belum ada data user.</p>
            </div>
          )}
        </div>
      </div>
