...
const InformasiBerkala = () => {
  ...
  const fetchInformasiBerkala = async () => {
    setIsLoading(true);
    try {
      const response = await getInformasiBerkala();
      setPpidData(response.data || []);
    } catch (error) {
      console.error("Error fetching informasi berkala:", error);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 md:mt-16">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-4 text-gray-500 font-medium tracking-wide">Memuat data...</p>
            </div>
            ...
            <div className="text-center py-32 bg-white rounded-3xl border border-dashed border-gray-200">
               <p className="text-gray-400 font-medium italic">Data informasi berkala belum tersedia.</p>
            </div>
          )}
        </div>
      </main>
