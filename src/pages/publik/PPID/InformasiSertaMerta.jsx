...
const InformasiSertaMerta = () => {
  ...
  const fetchInformasiSertaMerta = async () => {
    setIsLoading(true);
    try {
      const response = await getInformasiSertaMerta();
      setPpidData(response.data || []);
    } catch (error) {
      console.error("Error fetching informasi serta merta:", error);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {isLoading ? (
            ...
            <div className="text-center py-32 bg-white rounded-3xl border border-dashed border-gray-200">
              <p className="text-gray-400 font-medium italic">
                Data informasi serta merta belum tersedia.
              </p>
            </div>
          )}
        </div>
