...
const InformasiSetiapSaat = () => {
  ...
  const fetchInformasiSetiapSaat = async () => {
    setIsLoading(true);
    try {
      const response = await getInformasiSetiapSaat();
      setPpidData(response.data || []);
    } catch (error) {
      console.error("Error fetching informasi setiap saat:", error);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {isLoading ? (
                      ...
                      <tbody className="divide-y divide-gray-100 text-gray-600 text-sm md:text-base">
                        {section.dokumen.map((doc, dIdx) => (
                          ...
                          >
                            ...
                            <td className="py-4 md:py-5 px-6 md:px-8 text-gray-500">
            ...
            <div className="text-center py-32 bg-white rounded-3xl border border-dashed border-gray-200">
              <p className="text-gray-400 font-medium italic">
                Data informasi setiap saat belum tersedia.
              </p>
            </div>
          )}
        </div>
